...
def run_participant(bids_dir, session, shared,
    ...
    if os.path.exists(output_subdir):
        app.warn('Output directory for session "' + session_label + '" '
                 'already exists; all contents will be erased when this '
                 'execution completes')
        if app.ARGS.backup_parcel:
            app.console('backing up output directory')
            TIMESTAMP = datetime.datetime.now().strftime("%m%d%y%H%M%S%p")

            full_backup_path=output_subdir + '_' + TIMESTAMP    
            run.function(shutil.copytree,
                     output_subdir,
                     full_backup_path)
...
See the Mozilla Public License v. 2.0 for more details.''')
    ...
    if IS_CONTAINER:
        ...
        cmdline._action_groups[2].add_argument(
            ...
            version=__version__)
        cmdline._action_groups[2].add_argument(
            '-fsdir', '--freesurfer-dir',
            dest='freedir',
            help='Pass existing freesurfre directory')
        cmdline._action_groups[2].add_argument(
            '-bu', '--backup',
            dest='backup_parcel' ,
            action='store_true',
            help='Backup parcellation folder for previous run')
    else:
